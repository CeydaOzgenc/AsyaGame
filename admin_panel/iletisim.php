...
<?php
    ...
    if(!$_SESSION)
    {
        ...
        header("refresh:0;url=index.php");
            ...
            $sql=mysqli_fetch_array(mysqli_query($db,"select * from contact_info"));
            if(p("btngncl")=="Güncelle")
            {
                $adres=p("adres");
                $mail=p("mail");
                $telefon=p("telefon");
                $bak=mysqli_query($db,"select * from contact_info");
                    if(mysqli_num_rows($bak)==0)
                    {
                        echo "<script> alert ('Hatalı Giriş!'); </script>";
                    }
                    else
                    {
                        mysqli_query($db,"update contact_info set Info_location='".$adres."', Info_mail='".$mail."',    Info_call='".$telefon."'");
                                    ...
                                    <form method="post">
                                        ...
                                        <div class="card-body">
                                            <div class="col-xl-6 col-md-6" style="color:#000; margin: 2%;">
                                                <h3>Adres:</h3><br>
                                                <input class="form-control profil-form" type="text" name="adres" value="<?php echo $sql['Info_location']; ?>">
                                            </div>
                                            <div class="col-xl-6 col-md-6" style="color:#000;  margin: 2%;">
                                                <h3>E-Mail:</h3><br>
                                                <input class="form-control profil-form" type="text" name="mail" value="<?php echo $sql['Info_mail']; ?>">
                                            </div>
                                            <div class="col-xl-6 col-md-6" style="color:#000;  margin: 2%;">
                                                <h3>Telefon:</h3><br>
                                                <input class="form-control profil-form" type="text" name="telefon" value="<?php echo $sql['Info_call']; ?>">
                                            </div>
                                        </div>
                                        <div class="card-footer d-flex align-items-center justify-content-between">
                                            <input class="text-white" style="background: none; border:none;" type="submit" value="Güncelle" name="btngncl" />
