...
<?php
    ...
    if(!$_SESSION)
    {
        ...
        header("refresh:0;url=index.php");
            ...
            $sql=mysqli_fetch_array(mysqli_query($db,"select * from about"));
            if(p("btngnc")=="Güncelle")
            {
                $title=p("about_title");
                $text=p("about_text");
                if($_FILES["hakkinda_dosyasi"]["name"]!=""){
                    $dosyaad=$_FILES["hakkinda_dosyasi"]["name"];
                    $dosyayol="../assets/img/".$dosyaad;
                    $dtip=$_FILES["hakkinda_dosyasi"]["type"];
                    echo $dosyaad;
                    if($dtip="image/jpg" or $dtip="image/png" or $dtip="image/gif"){
                        if(is_uploaded_file($_FILES["hakkinda_dosyasi"]["tmp_name"])){
                            $tasi=move_uploaded_file($_FILES["hakkinda_dosyasi"]["tmp_name"],$dosyayol);
                            if($tasi){
                                unlink('../assets/img/'.$sql['About_img']);
                                mysqli_query($db,"update about set About_title='".$title."', About_text='".$text."', About_img='".$dosyaad."'");
                    ...
                    mysqli_query($db,"update about set About_title='".$title."', About_text='".$text."'");
                    header("refresh:0;url=hakkinda.php");
                }
            }
?>
                                    ...
                                    <form method="post" enctype="multipart/form-data">
                                        ...
                                        <div class="card-body">
                                            <div class="col-xl-6 col-md-6" style="color:#000; margin: 2%;">
                                                <h3>Başlık:</h3><br>
                                                <input class="form-control profil-form" type="text" name="about_title" value="<?php echo $sql['About_title']; ?>" />
                                            </div>
                                            <div class="col-xl-6 col-md-6" style="color:#000;  margin: 2%;">
                                                <h3>Yazı:</h3><br>
                                                <input class="form-control profil-form" type="text" name="about_text" value="<?php echo $sql['About_text']; ?>" />
                                            </div>
                                            <div class="col-xl-6 col-md-6" style="color:#000;  margin: 2%;">
                                                <h3>Fotoğraf:</h3><br>
                                                <img style="width: 30%; margin-bottom: 2%;" src="<?php echo '../assets/img/'.$sql['About_img'];?>"/>
                                                <input class="form-control profil-form" type="file" name="hakkinda_dosyasi" />
                                            </div>
                                        </div>
